...
- **Verified binary**: it downloads the standalone `gpc` binary from the GPC GitHub release and checks its SHA-256 on every run (cache hits included) against checksums committed in the action itself, not only the release's mutable `checksums.txt`. The default GPC version is pinned: 0.9.99 as of action v1.1.1. `gpc-version: latest` or an unpinned version falls back to the release checksums with a warning.
