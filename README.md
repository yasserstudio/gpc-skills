...
| **gpc-security** | Credential storage, key rotation, audit logging, supply chain protection (12 layers), Play App Signing with a self-hosted Cloud KMS key (`gpc app-signing`), incident response |
...
| **gpc-games** | **New in v1.26.0.** Play Games Services achievement and leaderboard configuration CRUD (`gpc games achievements/leaderboards` list/get/create/update/delete/diff), directory `push`/`pull`, `gpc games runtime` read-only, and `--game-id` resolution. |
...
| "Answer a Play chargeback dispute" | `gpc-monetization` |
| "What is a pendingRefundReviewNotification?" | `gpc-monetization` |
| "Why does `otp offers create` need --purchase-option?" | `gpc-monetization` |
| "Enroll Play App Signing with our own Cloud KMS key" | `gpc-security` |
| "Rotate our Play app signing key" | `gpc-security` |
| "Declare that a screenshot was generated by AI" | `gpc-metadata-sync` |
| "Play rejected my device tier config as unknown devices" | `gpc-release-flow` |
| "games set-icon fails with API_ENDPOINT_RETIRED" | `gpc-games` |
