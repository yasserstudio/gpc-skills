...
compatibility: "GPC v0.9+. Requires authenticated GPC setup (see gpc-setup skill). v0.9.96+ adds --ai-generated on listings images upload and sync."
metadata:
  version: 1.6.0
---
...
#### AI-generated image attestation (v0.9.96+)

Google Play asks developers to declare store imagery that was generated by AI. Pass `--ai-generated` to record that attestation with the upload; leave it off and the image is uploaded without a declaration. The flag is available on both `images upload` and `images sync`, and on `sync` it applies to **every** image the run uploads -- so split an AI-generated set into its own run rather than attesting a mixed directory.

```bash
gpc listings images upload ./screenshots/hero.png \
  --app com.example.app --lang en-US --type phoneScreenshots --ai-generated

gpc listings images sync --dir ./ai-screens --lang en-US --type phoneScreenshots --ai-generated
```
...
| `--ai-generated` | Attest that every image uploaded by this run was generated by AI (v0.9.96+) |
